...
<div class="absolute top-0 right-0 p-4 transition-transform h-full" class:translate-x-0={!opened} class:translate-x-full={opened}>
    <div class="h-full bg-slate-800 text-white rounded-lg tooz-shadow">
        <!-- EXTAND BUTTON -->
        <button
            class="absolute bottom-4 p-2 bg-slate-800 rounded-lg transition-all tooz-shadow"
            on:click={() => opened = !opened}
            class:right-[calc(100%+1rem)]={opened}
            class:right-full={!opened}
        >
            {#if opened}
