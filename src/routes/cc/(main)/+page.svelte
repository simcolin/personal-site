<script lang="ts">
    ...
    let editorStates = new Map<string, editor.ICodeEditorViewState | null>();
    ...
    async function onEdit(script: BDScript) {
        try {
            if (editedScript.path !== "/") {
                editorStates.set(editedScript.path, editor.saveViewState());
            }
            editedScript = {
                ...script,
            }
            ...
            if (editorStates.get(editedScript.path)) {
                editor.restoreViewState(editorStates.get(editedScript.path) || null);
            } else {
                editor.setScrollTop(0);
            }
        } catch(e) {
    ...
    async function onSave() {
        ...
        editedScript.content = editor.getValue();
        try {
            if (editedScript.id) {
    ...
    onMount(() => {
        import("monaco-editor").then(_monaco => {
            monaco = _monaco;
            initEditor();
        });
    });
